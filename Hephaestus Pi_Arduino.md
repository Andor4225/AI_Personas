...
__AI Project Context Definition: Raspberry Pi 5 Home Network Optimization \(Pi\-hole Journey\)__

__Meta Information__

- __Document Type__: Project Context & History
- __Last Updated__: July 23, 2025
- __Project Status__: Active \- Troubleshooting Unbound Installation
- __Primary Technology Stack__: Raspberry Pi 5, Pi\-hole, Raspberry Pi OS \(64\-bit\), Unbound \(pending\)

__Core Purpose of this Context \(for AI\)__

You are a dedicated AI assistant tasked with understanding, maintaining, and evolving the detailed history and current state of a user's specific home network project\. This project centers on transforming a Raspberry Pi 5 into a robust, private, and ad\-blocking DNS server \(Pi\-hole\), integrated securely within their home network infrastructure\. You must maintain a precise chronological log of all actions, challenges, and resolutions\.

__User Identity and Project Goals__

__User Profile:__

- SSH Username: andor4225
- Windows PC Username: Gavin
- Network Environment: Home network with AT&T BGW320\-500 Gateway router using IP passthrough to a Unifi Dream router
- Technical Level: Intermediate \(comfortable with SSH, basic Linux, networking concepts\)

__Primary Objective:__ To establish a stable, secure, and private home network that effectively blocks advertisements and trackers across all devices using Pi\-hole, running on a Raspberry Pi 5\.

__Secondary Objectives:__

- Maximize system stability
- Enhance security against external threats
- Achieve maximum DNS privacy \(recursive resolution via Unbound\)

__I\. Initial Project State and Ambitions \(The First Iteration \- Pre\-Wipe Context\)__

__Initial Hardware/Software State:__

- Raspberry Pi 5 connected via Ethernet
- Running Raspberry Pi OS from Micro SD card
- Pi\-hole already installed \(basic configuration\)

__Initial Request and Goals:__ The user sought guidance to:

1. Customize and improve their existing Pi\-hole installation
2. Enhance the overall security of their Raspberry Pi
3. Explore advanced features and performance optimizations

__Key Proposed Enhancements \(Initial Phase\):__

- __DNS Recursion__: Using Unbound for enhanced privacy \(avoiding third\-party DNS servers\)
- __Network Segmentation__: Pi\-hole Group Management for differentiated filtering policies
- __Local Network Resolution__: Configuring Local DNS Records and Conditional Forwarding
- __Security Best Practices \(Initial Outline\)__: 
	- Change Pi\-hole web password
	- Regular OS updates
	- Comprehensive SSH security \(disable root login, SSH key authentication, custom port\)
	- Implement host\-based firewall \(UFW\)
	- Set up brute\-force prevention tool \(Fail2Ban\)

__II\. Phase 1: Initial Setup & Troubleshooting Challenges \(The First Attempt at Full Deployment\)__

__A\. Router DNS Configuration Limitation__

__\[PROBLEM\]__ AT&T BGW320\-500 Gateway router lacks global DNS configuration option

- __Impact__: Devices won't automatically use Pi\-hole unless individually configured or Pi\-hole takes over DHCP
- __Initial Solution Attempt__: Enable Pi\-hole's built\-in DHCP server; disable router's DHCP
- __Result__: Partial success, led to new issues

__B\. Fire TV DNS Bypass & Router Firewall__

__\[PROBLEM\]__ Streaming devices \(Amazon Fire TVs\) bypassing Pi\-hole

- __Symptoms__: Intermittent connectivity, streaming failures \(Prime Video, ESPN\)
- __Root Cause__: Hardcoded DNS \(8\.8\.8\.8, IPv6 addresses\)
- __Initial Solution Attempt__: Router firewall "Drop" rules blocking port 53 outbound
- __Result__: Over\-blocking, causing more severe connectivity issues

__\[CLARIFICATION POINT\]__ Temporary confusion about Pi\-hole DHCP state during troubleshooting

__\[CRITICAL ISSUE\]__ Pi\-hole's DHCP server exhibited instability and crashes

- __Symptoms__: Loss of web interface access, network disruption
- __Decision__: Disable Pi\-hole DHCP, use manual DNS configuration per device

__C\. Pi\-hole Upstream DNS & Router Firewall \(Re\-evaluation\)__

__\[PROBLEM\]__ Router firewall blocking Pi\-hole's own upstream queries

- __Log Evidence__: Forwarded to 1\.0\.0\.1\#53 but Reply: No reply received
- __Affected Services__: Smart lights \(device\-api\.ailofy\.com\), Amazon services

__\[SOLUTION \- SUCCESS\]__ Created specific firewall rules:

Pass Rules \(Higher Priority\):

\- Source: 192\.168\.10\.2 \(Pi\-hole\)

\- Destination: 1\.0\.0\.0/8 \(Cloudflare\)

\- Port: 53 \(TCP/UDP\)

Drop Rules \(Lower Priority\):
...
- __Result__: ✅ Ailofy lights and Alexa connectivity restored

__III\. Phase 2: The Major Lockout & Forced Re\-Flash Decision__

__A\. Persistent SSH Agent Failures \(Deep Windows\-Specific Issues\)__

__\[PROBLEM 1\]__ SSH Private Key File Permissions

- __Environment__: Windows 10/11, Git Bash
- __Issue__: chmod 600 ~/\.ssh/id\_ed25519 ineffective
- __Symptoms__: Permission denied \(publickey\)
- __Root Cause__: NTFS permissions interference, Windows ACLs

__\[PROBLEM 2\]__ SSH Agent Persistence & Integration

- __Failed Attempts__: 
	- Git Credential Manager \- no pop\-up
	- Windows OpenSSH Authentication Agent \- no connection from Git Bash
	- Pageant \(PuTTY\) \- loaded key but Git Bash couldn't retrieve passphrase
- __Root Cause__: Windows SSH client/agent implementation conflicts

__B\. Critical SSH Lockouts__

__\[CRITICAL FAILURE\]__ Complete remote access loss

- __Contributing Factors__: 
	- SSH hardening \(disabled password auth\)
	- SSH key/agent failures
	- No physical access \(lacking mini\-HDMI cable\)

__\[RECOVERY ATTEMPT \- FAILED\]__ Mount Pi SSD via USB to Windows/WSL

- __Issue__: wsl \-\-mount commands failed
- __Impact__: Cannot edit sshd\_config to restore access

__C\. Decision to Re\-Flash__

__\[DECISION POINT\]__ Complete system re\-flash chosen

- __Rationale__: Only reliable method to regain control
- __Trade\-off__: Loss of all Pi\-hole configuration

__IV\. Phase 3: The Fresh Start & Current Stable State__

__A\. SSD Re\-Flash & Initial Setup__

__\[SUCCESS\]__ Clean installation achieved

- __Storage__: Western Digital SN550 NVMe SSD \(via USB adapter\)
- __OS__: Raspberry Pi OS 64\-bit
- __Key Learning Applied__: SSH enabled with password auth only \(stability first\)
- __Network Config__: DHCP reservation at 192\.168\.10\.2

__B\. Pi\-hole Re\-Installation & Core Functionality__

__\[SUCCESS\]__ Pi\-hole operational

- __Upstream DNS__: Cloudflare \(1\.1\.1\.1/1\.0\.0\.1\)
- __Web Password__: Set via pihole \-a setpassword
- __DHCP Status__: Disabled \(stability choice\)
- __Router Firewall__: Pass/Drop rules re\-applied successfully

__C\. SSH Hardening \(Partial\) & System Security__

__\[CURRENT STATE\]__

- ✅ SSH Access: Stable, password auth, port 22
- ✅ OS Updates: Current
- ❌ Fail2Ban: Removed \(installation issues\)
- ⚠️ CrowdSec: Core installed, bouncers unavailable
- ✅ UFW: Configured and active

__D\. Unbound \(Current Challenge\)__

__\[ACTIVE ISSUE\]__ unbound\-anchor: command not found

- __Package Status__: Reports "already the newest version"
- __Likely Cause__: Package integrity or PATH issue
- __Next Steps__: Purge and reinstall

__V\. Current System State \(July 23, 2025\)__
...
Western Digital SN550 NVMe SSD \(USB boot\)
...
192\.168\.1\.119 \(DHCP reservation\)
...
Password auth, port 22, user: andor4225

Pi\-hole

✅ Active

Web UI: http://192\.168\.10\.2/admin

Pi\-hole DHCP

❌ Disabled

Manual DNS config per device

UFW Firewall

✅ Active

Allows: SSH, DNS, HTTP/S
...
Pass rules for Pi → Cloudflare; Drop all other DNS

Manual DNS Devices

Computer, 2 cell phones → 192\.168\.1\.119

__VI\. Key Learnings & Mistakes \(To Avoid Recurrence\)__

__Critical Lessons Learned__

1. __Over\-Hardening Too Early__ ❌
	- __Mistake__: SSH hardening before stable key access
	- __Lesson__: Establish basic access first, harden incrementally
	- __Status__: Applied in current setup
2. __Windows SSH Environment Complexity__ ⚠️
	- __Issue__: Permission management, agent integration
	- __Lesson__: Consider WSL as primary SSH environment
	- __Status__: Unresolved, using password auth
3. __Package Installation Verification__ ✅
	- __Mistake__: Assuming apt install success
	- __Lesson__: Always verify with systemctl status or which
	- __Status__: Now standard practice
4. __CLI Output Interpretation__ ✅
	- __Mistake__: Misreading help output as confirmation
	- __Lesson__: Verify actions independently
	- __Status__: Resolved
5. __Physical Access Planning__ ⚠️
	- __Issue__: No monitor/keyboard fallback
	- __Lesson__: Always have console access plan
	- __Status__: Still lacking mini\-HDMI cable
6. __Firewall Rule Ordering__ ✅
	- __Learning__: Pass rules must precede Drop rules
	- __Status__: Successfully implemented

__Common Command Reference__

\# SSH Access

ssh andor4225@192\.168\.10\.2
...
dig @192\.168\.1\.119 google\.com

nslookup google\.com 192\.168\.1\.119

__Current Troubleshooting Focus__

__Unbound Installation Issue__

\# Diagnostic commands to run:

which unbound\-anchor

dpkg \-L unbound | grep anchor
...
__Project Evolution Summary__

1. __Initial Setup__ → Router limitations discovered → DHCP takeover attempted
2. __DHCP Instability__ → Manual DNS configuration → Firewall rules refined
3. __SSH Lockout__ → System re\-flash → Stability\-first approach
4. __Current State__ → Core functionality stable → Working on Unbound recursion

This comprehensive context should equip any AI to seamlessly pick up where we left off, understanding the full history and the unique challenges of your Raspberry Pi project\.
